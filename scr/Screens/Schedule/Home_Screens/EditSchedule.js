...
const EditSchedule = ({ navigation, route }) => {
    const { schedule, updateSchedule } = useDataSchedule();
    const item = route.params?.item || "Null";
    const now = moment().valueOf();
    const [DateModalS, setDateModalS] = useState(false);
    const [valueDateS, setValueDateS] = useState(item.dateS);
    const [TimeModalS, setTimeModalS] = useState(false);
    const [valueTimeS, setValueTimeS] = useState(item.timeS);
    const [DateModalE, setDateModalE] = useState(false);
    const [valueDateE, setValueDateE] = useState(item.dateE);
    const [TimeModalE, setTimeModalE] = useState(false);
    const [valueTimeE, setValueTimeE] = useState(item.timeE);

    const [alarm, setAlarm] = useState(item.alarm);
    ...
    const handleAdd = () => {
        const timeS = valueTimeS!==item.timeS ? formatTime(valueTimeS) : valueTimeS;
        const dateS = valueDateS!==item.dateS ? formatDate(valueDateS) : valueDateS;
        const timeE = valueTimeE!==item.timeE ? formatTime(valueTimeE) : valueTimeE;
        const dateE = valueDateE!==item.dateE ? formatDate(valueDateE) : valueDateE;
        const it = {
            id: item.id,
            title: title,
            dateS: dateS,
    ...
    useEffect(() => {
        ...
        setValueDateS(item.dateS);
        setValueTimeS(item.timeS);
        setValueDateE(item.dateE);
        setValueTimeE(item.timeE);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white', flexDirection: 'column', padding: 10 }}>
            <View style={{ flexDirection: 'row', margin: 10, justifyContent: 'center', alignContent: 'center' }}>
                ...
                <Text style={{ color: '#000', fontSize: 25, fontWeight:'bold'}}>Chỉnh sửa lịch trình</Text>
